-- =============================================================================
-- JCI ERODE METRO — COMPLETE LIVE DATABASE SCHEMA (END-TO-END)
-- =============================================================================
-- File: jci_erode_metro_live_setup.sql
...
--   mysql -u root -p < jci_erode_metro_live_setup.sql
--
-- Backend .env must match:
--   HS_DB_NAME=api_jcierodemetro
--   HS_DB_USERNAME=jcierodemetro
--   HS_DB_PASSWORD=4JkxCCQB8QfVP3sczfSrH3koO
--   HS_PORT=3029
--   HS_IMAGE=https://api.jcierodemetro.com
--
-- Admin login: admin@jci.local / Admin@12345 (change after first login)
--
-- WARNING: Drops database api_jcierodemetro if it exists (DESTROYS ALL DATA).
-- After this file, run add_members_erode_metro_registration.sql to import members.
-- =============================================================================

-- =============================================================================
-- PART 1: SCHEMA + MINIMAL SEED (admin, roles, business types only)
-- =============================================================================
-- Tables match Sequelize freezeTableName (Member, Family, boardMembers, etc.)
-- HOW TO RUN: Press Ctrl+A to select ALL, then click Execute (lightning bolt)

DROP DATABASE IF EXISTS `api_jcierodemetro`;
CREATE DATABASE `api_jcierodemetro` CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
...
CREATE TABLE `MemberNotification` (
  `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT,
  `member_id` bigint UNSIGNED NOT NULL,
  `type` enum('referral_received','referral_viewed','referral_responded') NOT NULL,
  `title` varchar(255) NOT NULL,
  ...
  `updatedAt` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_member_notification_member` (`member_id`),
  KEY `idx_member_notification_read` (`is_read`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

SET FOREIGN_KEY_CHECKS = 1;

SELECT 'SUCCESS: JCI Erode Metro database schema installed.' AS Status;
-- =============================================================================
-- PART 2: PRODUCTION MYSQL APP USER (jcierodemetro)
-- =============================================================================

CREATE USER IF NOT EXISTS 'jcierodemetro'@'localhost' IDENTIFIED BY '4JkxCCQB8QfVP3sczfSrH3koO';
CREATE USER IF NOT EXISTS 'jcierodemetro'@'127.0.0.1' IDENTIFIED BY '4JkxCCQB8QfVP3sczfSrH3koO';
-- If backend runs on a different host than MySQL, also grant from that host:
-- CREATE USER IF NOT EXISTS 'jcierodemetro'@'10.0.0.5' IDENTIFIED BY '4JkxCCQB8QfVP3sczfSrH3koO';

GRANT ALL PRIVILEGES ON api_jcierodemetro.* TO 'jcierodemetro'@'localhost';
GRANT ALL PRIVILEGES ON api_jcierodemetro.* TO 'jcierodemetro'@'127.0.0.1';
FLUSH PRIVILEGES;
...
SELECT 'Tables' AS check_type, COUNT(*) AS value FROM information_schema.tables WHERE table_schema = 'api_jcierodemetro';
SELECT 'Admin users' AS check_type, COUNT(*) AS value FROM `Admin`;
SELECT 'Members' AS check_type, COUNT(*) AS value FROM `Member`;
SELECT 'Referral module' AS check_type, IF(COUNT(*) > 0, 'OK', 'MISSING') AS value FROM information_schema.tables WHERE table_schema = 'api_jcierodemetro' AND table_name = 'Referral';
SELECT 'Fitness module' AS check_type, IF(COUNT(*) > 0, 'OK', 'MISSING') AS value FROM information_schema.tables WHERE table_schema = 'api_jcierodemetro' AND table_name = 'FitnessStory';
SELECT 'Member auth' AS check_type, IF(COUNT(*) > 0, 'OK', 'MISSING') AS value FROM information_schema.tables WHERE table_schema = 'api_jcierodemetro' AND table_name = 'MemberAuth';
SELECT 'Member notifications' AS check_type, IF(COUNT(*) > 0, 'OK', 'MISSING') AS value FROM information_schema.tables WHERE table_schema = 'api_jcierodemetro' AND table_name = 'MemberNotification';

SELECT 'SUCCESS: JCI Erode Metro live DB is ready. Next: import add_members_erode_metro_registration.sql' AS Final_Status;
