...
- Refactor all files from className components to functional ones, so now, you can use React Hooks
...
- All `div`'s with Bootstrap className (`.container`,`.container-fluid`,`.row`,`.col-*`) were changed in React-Bootstrap components
- Changed `Notifications Places` buttons from buttons with `className` to our own button component `CustomButton`
- Added property `round` in `CustomButton` (for the button className `.btn-round`)
